...
- ユーザは状態を持つ。毒と通常状態
- ユーザはヒットポイントが0になると死亡する
- 毒の状態だと一定時間毎に10のダメージを受ける

現在のステータスは以下のようになっています。`USER_A`は毒の状態です。
...
同様に2.の処理の前に`USER_A`のデータの排他ロックを取得します。
ただし、1.の処理が終わってないので、それが終わる(ロックが解除される)まで待ちます。1.の処理が終わった後には`status`が`normal`になってるので、`USER_A`が毒のダメージを受けません。
