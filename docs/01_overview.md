...
そういった場合にデータを適切に更新するための仕組みがロックとトランザクションです。

### ロック

RPGゲームを例に以下の場合を考えます。

1. `USER_A`が`hit_point`を100回復します。
1. `USER_B`が`USER_A`に120のダメージを与えます。


`hit_pointテーブル`

    +---------+------------+---------------+
    | user    | hit_point  | max_hit_point |
    +---------+------------+---------------+
    | USER_A  | 100        | 300           |
    | USER_B  | 300        | 300           |
    +---------+------------+---------------+

通常であれば、この2つの処理が終わった後の`USER_A`の`hit_point`は80となります。
しかしながら、2.を処理するにあたって元にした`USER_A`の`hit_point`が1.の実行前であれば、
最終的な`USER_A`の`hit_point`は0(-20)となります。(`hit_poit`のデータ型による)

こういったことを避けるために、1.の処理の前にロックを取得し、処理が終わったら解除します。
また、同様に2.の処理前にロックを取得します。
...
データの一貫性を保つために複数の処理を1つの単位として不可分に行うもの。
...
1. `USER_A`が`USER_B`の口座に10,000振り込む
    ...
    1. `USER_B`の口座を10,000増やす
1. `USER_B`が`USER_A`の口座に20,000振り込む
    1. `USER_B`の口座を20,000減らす
    1. `USER_A`の口座を20,000増やす

この状態で下記の順番で処理が実行されるとデッドロックが生じます。

1. 1-1 <= `USER_A`の口座がロックされる
1. 2-1 <= `USER_B`の口座がロックされる
1. 1-2 <= `USER_B`の口座をロックしようとする(ロック開放待ち)
1. 2-2 <= `USER_A`の口座をロックしようとする(ロック開放待ち) => デッドロック!!
